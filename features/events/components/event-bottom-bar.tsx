...
import { formatEventPrice } from "@/features/events/format"
...
export function EventBottomBar({ event }: { event: PublicEvent }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-foreground/10 bg-background/95 backdrop-blur">
      <div className="mx-auto flex w-full max-w-3xl items-center gap-3 px-6 py-3">
        <div className="hidden min-w-0 flex-1 sm:block">
          <p className="text-[11px] tracking-[0.14em] text-foreground/45 uppercase">
            Tickets
          </p>
          <p className="truncate font-sans-semibold text-foreground">{price}</p>
        </div>
        <StoreButton
          store="apple"
